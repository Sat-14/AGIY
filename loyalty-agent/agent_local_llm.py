...
# Initialize local LLM for loyalty agent (Phi-2 - strong at math/logic)
llm_manager = LocalLLMManager(M1_8GB_CONFIGS["loyalty"])
...
def calculate_best_offers_with_llm(user_loyalty, cart_amount, segment_offer, available_coupons):
    """
    Use LLM (Phi-2) to intelligently recommend best offers.
    Phi-2 is excellent at mathematical reasoning and logic.
    """
    loyalty_points_value = user_loyalty["loyaltyPoints"] * user_loyalty["pointValue"]

    # Prepare context for LLM
    prompt = M1_AGENT_PROMPTS["loyalty"].format(
        tier=user_loyalty.get("tier", "regular"),
        cart_amount=cart_amount,
        loyalty_points=user_loyalty["loyaltyPoints"]
    )

    context = f"""User Profile:
- Tier: {user_loyalty.get("tier", "regular")}
- Loyalty Points: {user_loyalty["loyaltyPoints"]}
- Point Value: ₹{loyalty_points_value}
- Cart Amount: ₹{cart_amount}

Segment Offer: {segment_offer["description"]}

Available Coupons:
{json.dumps(available_coupons, indent=2)}

Task: Recommend the best combination to maximize savings."""
    ...
    try:
        ...
        try:
            ...
            if "recommendations" in llm_suggestions:
                return llm_suggestions["recommendations"]
        except json.JSONDecodeError:
            # LLM returned text, extract insights
            pass
    ...
    recommendations = [
        ...
        f"As a {user_loyalty.get('tier', 'regular')} member, you get {segment_offer['description']}"
    ]

    # LLM-enhanced suggestion
    if cart_amount > 1000 and "FLAT100" in [c["offerId"] for c in available_coupons]:
        recommendations.insert(0, "💡 Best Deal: Use FLAT100 coupon + redeem 500 points for maximum savings!")
...
def get_applicable_offers():
    ...
    Enhanced with LLM recommendations.
    """
    ...
    loyalty_points_value = user_loyalty["loyaltyPoints"] * user_loyalty["pointValue"]
    is_redeemable = user_loyalty["loyaltyPoints"] >= 100

    # Get LLM-enhanced recommendations
    llm_recommendations = calculate_best_offers_with_llm(
        user_loyalty, cart_amount, segment_offer, available_coupons
    )
    ...
    llm_manager.collect_feedback(
        prompt=f"offers_user_{user_id}",
        response=json.dumps(response),
        rating=5,
        metadata={"user_id": user_id, "cart_amount": cart_amount}
    )
...
def feedback_endpoint():
    ...
    llm_manager.collect_feedback(
        prompt=f"loyalty_user_{data.get('userId')}",
        response=data.get('action', 'offer_query'),
...
def health_check():
    ...
    return jsonify({
        "status": "healthy",
        "agent": "loyalty",
        "llm_model": M1_8GB_CONFIGS["loyalty"].model_name,
        "memory_limit_mb": M1_8GB_CONFIGS["loyalty"].max_memory_mb
    })

if __name__ == '__main__':
    print("\n" + "="*70)
    print("Loyalty & Offers Agent with Local LLM (Phi-2 2.7B)")
    print("="*70)
    print(f"Model: {M1_8GB_CONFIGS['loyalty'].model_name}")
    print(f"Memory Limit: {M1_8GB_CONFIGS['loyalty'].max_memory_mb} MB")
    print(f"Quantization: {M1_8GB_CONFIGS['loyalty'].quantization}")
    print(f"Specialization: Mathematical reasoning & logic")
    ...
    print("--- Loyalty & Offers Agent (Flask API) Starting on Port 5006 ---")
